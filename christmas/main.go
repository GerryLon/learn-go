...
func (ct *ChristmasTree) gen() {
	...
	for floor := 0; floor < ct.floor; floor++ {
		for line := 0; line < floor+5; line++ {
			...
			for i := (bottomAmount-lineAmount)/2 - 1; i > 0; i-- {
				ct.treeBuilder.WriteString(WidthSpace)
			}
	...
	if ct.floor > 3 {
		...
		lines[len(lines)-4] += strings.Repeat(WidthSpace, 4) + GIFT
		lines[len(lines)-3] += strings.Repeat(WidthSpace, 3) + GIFT + GIFT + GIFT
		lines[len(lines)-2] += strings.Repeat(WidthSpace, 2) + GIFT + GIFT + GIFT + GIFT + GIFT
